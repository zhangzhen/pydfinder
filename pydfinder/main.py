...
class SoftClippingRead(object):
    ...
    def len_consistent(self, deletion, isize):
        if self.map_dist <= len(deletion):
            return False
        return isize.len_in_normal_range(self.map_dist - len(deletion))
...
class ThreePrimeEndForward(SoftClippingRead):
    ...
    def close_to(self, deletion, slop):
        return make_gregion(deletion.start(), slop).in_region(self.gpos)
...
class ThreePrimeEndReverse(SoftClippingRead):
    ...
    def close_to(self, deletion, slop):
        return make_gregion(deletion.end(), slop).in_region(self.gpos)
...
def check_support(sc_read, deletion, slop, isize):
    return sc_read.len_consistent(deletion, isize) and sc_read.close_to(deletion, slop)
